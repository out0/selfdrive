...
class HermiteCurve:
    ...
    def interpolate(width: int, height: int, p1: tuple[int, int, float], p2: tuple[int, int, float]):
        ...
        for i in range(0, num_points):
            ...
            k = abs(ddx*dd2z - ddz*dd2x) / math.pow(ddx ** 2 + ddz**2, 3/2)
            print (f"curvature: k={k}")
            hv = k > 0.30


            curve.append((cx, cz, heading, hv))
            lastp = (cx, cz)
