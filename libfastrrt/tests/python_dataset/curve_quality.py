...
class CurveData:
    ...
    def to_csv_header(self) -> str:
        return "\"name\";\"num_points\"; \"jerk\"; \"total_length\"; \"tan_discontinuity\"; \"coarse\"; \"proc_time_ms\"; \"timeout\"; \"num_loops\";\"goal_reached\";\"total_cost\"\n"
...
class CurveAssessment:
    ...
    def __compute_curve_cost(cpu_frame: np.ndarray, curve: np.ndarray) -> float:
        cost = 0.0
        for i in range(curve.shape[0]):
            x = int(curve[i, 0])
            z = int(curve[i, 1])
            cost += float(cpu_frame[z, x, 1])
        return cost

    
    def assess_curve(self, cpu_frame: np.ndarray, curve: np.ndarray, start_heading: float, compute_heading: bool = True) -> CurveData:
        return CurveData(
            name=None,
            coarse=False,
            goal_reached=False,
            ...
            total_cost=CurveAssessment.__compute_curve_cost(cpu_frame, curve),
        )
