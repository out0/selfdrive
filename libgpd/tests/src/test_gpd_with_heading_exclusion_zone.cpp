...
TEST(GPD_WithHeading, Test_InRange_NoObstacles_ExclusionZone)
{
    return;
    ...
    /// in this test, all of the points are feasible. Thus, the GPD must find the exact L1 as provided
    ...
    for (int z = 0; z < 100; z++)
        for (int x = 0; x < 100; x++)
            for (int h = -90; h <= 90; h += 45)
            {
                ...
                gpd.computeExclusionZone(f1, heading);
...
TEST(GPD_WithHeading, Test_InRange_InvalidL1_Straight_ExclusionZone)
{
    GoalPointDiscover gpd;
    ...
    gpd.computeExclusionZone(f1, angle::deg(0.0));

    Waypoint p = gpd.findLowestCostWaypointWithHeading(f1, {10, 10}, 50, 0, 0.0);
    ASSERT_TRUE(std::abs(p.x() - 50) >= 5);
    ASSERT_TRUE(f1.isTraversable(p.x(), p.z(), p.heading(), true));
}

TEST(GPD_WithHeading, Test_InRange_InvalidL1_Curve_ExclusionZone)
{
    GoalPointDiscover gpd;
    ...
    f1.processSafeDistanceZone({10, 10}, true);
    gpd.computeExclusionZone(f1, angle::deg(-45.0));

    Waypoint p = gpd.findLowestCostWaypointWithHeading(f1, {10, 10}, 0, 21, angle::deg(-45).rad());
    ASSERT_EQ(p.x(), 0);
    ASSERT_TRUE(std::abs(p.z() - 21) >= 5);
    ASSERT_TRUE(f1.isTraversable(p.x(), p.z(), p.heading(), true));
}

TEST(GPD_WithHeading, Test_Nearby_NoObstacles_ExclusionZone)
{
    return;
    GoalPointDiscover gpd;
...
TEST(GPD_WithHeading, Test_Nearby_InvalidL1_Straight_ExclusionZone)
{
    GoalPointDiscover gpd;
...
TEST(GPD_WithHeading, Test_Nearby_InvalidL1_Curve_ExclusionZone)
{
    GoalPointDiscover gpd;
    ...
    f1.processSafeDistanceZone({10, 10}, false);
    gpd.computeExclusionZone(f1, angle::deg(-45));

    Waypoint p = gpd.findLowestCostWaypointWithHeading(f1, {10, 10}, -10, 21, angle::deg(-45).rad());
    ASSERT_TRUE(f1.isTraversable(p.x(), p.z(), p.heading(), true));
}

TEST(GPD_WithHeading, Test_Far_NoObstacles_ExclusionZone)
{
    return;
    GoalPointDiscover gpd;
...
TEST(GPD_WithHeading, Test_Far_InvalidL1_Straight_ExclusionZone)
{
    GoalPointDiscover gpd;
...
TEST(GPD_WithHeading, Test_Far_InvalidL1_Curve_ExclusionZone)
{
    GoalPointDiscover gpd;
