...
class TestCoordinateConverterWorldMap(unittest.TestCase):
    ...
    def __test_conversion_to_map(self, conv: CoordinateConverter, pose: str, map_truth: str):
        ...
        #self.assertAlmostEqual(map_pose.heading.rad(), map_pose_carla.heading.rad(), 0)
        self.assertTrue(map_pose.heading == map_pose_carla.heading)
        
    def __test_conversion_to_world(self, conv: CoordinateConverter, pose: str, map_truth: str):
        world_pose_truth: WorldPose = WorldPose.from_str(pose)
        map_pose: MapPose = MapPose.from_str(map_truth)
        world_pose : WorldPose = conv.convertWorld(map_pose)
        
        self.assertAlmostEqual(world_pose_truth.lat.rad(), world_pose.lat.rad(), 7)
        self.assertAlmostEqual(world_pose_truth.lon.rad(), world_pose.lon.rad(), 7)
        self.assertTrue(world_pose_truth.compass == world_pose.compass)
    ...
    def test_relative_conversion(self):
        return
        w1: WorldPose = WorldPose.from_str("-4.256008878655848e-09|-1.5864868596990013e-08|1.0150023698806763|89.99923752329113")
        conv = CoordinateConverter(w1, 
                                              OG_WIDTH,
                                              OG_HEIGHT,
                                              OG_REAL_WIDTH,
                                              OG_REAL_HEIGHT)
        map: MapPose = conv.convertWorld(w1)

        self.assertAlmostEqual(map.x, 0, places=5)
        self.assertAlmostEqual(map.y, 0, places=5)
        self.assertAlmostEqual(map.z, 0, places=5)
        self.assertAlmostEqual(map.heading.rad(), 0, places=5)

        map2 = map + 10
        w2: WorldPose = conv.convertWorld(map2)

        map2 : MapPose = conv.convertWorld(w2)
        ...
        self.assertAlmostEqual(map2.heading.rad(), 0, places=5)
