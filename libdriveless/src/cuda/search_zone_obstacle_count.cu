...
__device__ __host__ inline int2 zone_location(int2 zone_dim_size, int2 zone_grid_size, int x, int z)
{
...
__device__ __host__ void count_obstacle_in_search_zones(float3 *frame, float *classCosts, int *search_params, uint2 *search_zone_info, int pos)
{
    ...
    if (classCosts[segmentation_class] < 0)
    {
        ...
        const int A = location.x * search_zone_dim_w;
        const int B = location.y * search_zone_dim_h;
        ...
        if ((x >= A && x <= (A + search_zone_dim_w - 1)) && (z == B || z == (B + search_zone_dim_h - 1)))
            atomicInc(&search_zone_info[posg].y, 99999999);

        else if ((z >= B && z <= (B + search_zone_dim_h - 1)) && (x == A || x == (A + search_zone_dim_w - 1)))
            atomicInc(&search_zone_info[posg].y, 99999999);
#else
