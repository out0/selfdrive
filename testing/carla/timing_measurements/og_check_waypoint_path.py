...
def execute_plan (seq: int) -> bool:
    ...
    path = duplicate_path(result.path)

    path = limit_path_size(path, 140)
    if path is None:
        return
    
    ExecutionTimeRecorder.initialize(file="./measurements.log")
    ExecutionTimeRecorder.start(f"path_check_{seq}")    
    og.check_path_feasible(path)
    ExecutionTimeRecorder.stop(f"path_check_{seq}")
    return True
