...
class CurveAssessment:
    ...
    def assess_curve(curve: list[Waypoint]) -> CurveData:
        if (len(curve) == 0):
            return None
        ...
        return CurveData(
            ...
            tan_discontinuity=CurveAssessment.__tangential_discontinuity(
                curve, window_side=4, threshold=15, 
                start_heading=curve[0].heading, compute_heading=False),
            last_p=curve[-1]
        )
    ...
    def __tangential_discontinuity(curve: list[Waypoint], window_side: int=2, threshold: float = 15, start_heading: float = 0.0, compute_heading: bool = True):
        ...
        for i in range(1, len(curve)):
            if compute_heading:
                ...
                a = curve[i].heading
            if abs((a - a_before).rad()) > threshold_rad:
                return True
