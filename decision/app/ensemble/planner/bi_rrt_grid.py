...
class NodeGrid:
    ...
    _lower_bound: int2
    _upper_bound: int2
    _physical_params: GridPhysicalParameters
    ...
    def check_feasible(self, og: SearchFrame, p: int2, heading_deg: float) -> bool:
        return og.is_traversable(p[0], p[1], angle.new_deg(heading_deg), precision_check=False)
    ...
    def set_physical_params(self, physical_params: GridPhysicalParameters) -> None:
    ...
    def check_direct_connection(self, frame: SearchFrame, start: Waypoint, end: Waypoint) -> bool:        
        curve = Interpolator.hermite(self._width, self._height, start, end, return_as_waypoint=True)
        return frame.check_feasible_path(self._min_dist, curve, False)
    ...
    def check_kinematic_connection(
        ...
        while curr_dist < DISTANCE:
            ...
            path.append(Waypoint(x=nextp[0], z=nextp[1], heading=angle.new_rad(heading)))
            curr_cost += self.get_intrinsic_cost(frame, nextp) + 1
            curr_dist += 1

        if euclidean_distance(lastp, end) <= 2:
            if frame.check_feasible_path(self._min_dist, path, individual_waypoint_check=False):
