import math
import numpy as np
from pydriveless import Waypoint, angle, HALF_PI
...
class ReedsShepp:
	...
	"""
	step: float
	def __init__(self, step: float, vehicle_length_m: float, max_steering_angle: float, speed: float) -> None:
		self.step = step
		t = math.tan(math.radians(max_steering_angle))
		self.max_curv = t/(speed*vehicle_length_m)
	...
	def generation(self, start_pose: Waypoint, goal_pose: Waypoint):
		"""
		...
		"""
		sx, sy, syaw = start_pose.x, start_pose.z, start_pose.heading.rad()
		gx, gy, gyaw = goal_pose.x, goal_pose.z, goal_pose.heading.rad()
