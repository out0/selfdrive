...
class TestFastRRT(unittest.TestCase):
    ...
    def hermite_interpolation(width: int, height: int, p1: tuple[int, int, float], p2: tuple[int, int, float]) -> list[int, int]:     
        curve =[]
    ...
    def test_small_cluttered(self):
        ...
        hermite = TestFastRRT.hermite_interpolation(
            width=data.width(),
            height=data.height(),
            p1=start,
            p2=goal            
        ...
        ideal_path = np.ndarray((len(hermite), 3), dtype=np.float32)
        i = 0
        for p in hermite:
            ideal_path[i, 0] = p[0]
            ideal_path[i, 1] = p[1]
        ...
        ideal_points = np.zeros((data.height(),2), dtype=np.int32)
        for p in hermite:
            ideal_points[p[1], 0] = p[0]
            ideal_points[p[1], 1] = 1

        for _ in range(100):
            total_cost = 0
            TestUtils.output_2path_result(data.frame, path, ideal_path, "output1.png")

            for p in path:
                x = int(p[0])
                z = int(p[1])
                ideal_x, valid = ideal_points[z, :]
                if valid == 0: continue
                ...
                if d != 0:
                    if abs(d) == 1:
                        ...
                        x = int(round(0.5 * (x + ideal_x)))
    
                    p[0] = x
                    total_cost += abs(ideal_x - x)
